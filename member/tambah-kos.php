<?php
	...
	if(@$id_kos){
		$data = $konek->query("SELECT kos.*, gambar_kos.* FROM kos, kategori, gambar_kos WHERE kategori.id_kat=kos.id_kat AND gambar_kos.id_kos=kos.id_kos AND kos.id_kos='$id_kos' AND kos.id_akun='".$_SESSION['id']."'");
		if($data->num_rows > 0){
